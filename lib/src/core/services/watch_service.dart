...
class WatchService extends GetxService {
  ...
  Future<void> sendUpdateObsConnecteToNative({
    ...
    if (!await _ensureChannelAvailable()) {
      talker.warning('Watch channel unavailable; skipping sendUpdateObsConnecteToNative');
      return;
    }
    try {
      await _channel.invokeMethod("flutterToWatch", {
  ...
  Future<void> sendSelectedObsSceneToNative(String sceneName) async {
    if (!await _ensureChannelAvailable()) {
      talker.warning('Watch channel unavailable; skipping sendSelectedObsSceneToNative');
      return;
    }
    try {
      await _channel.invokeMethod("flutterToWatch", {
  ...
  Future<void> sendObsScenesToNative(List<String> scenes) async {
    if (!await _ensureChannelAvailable()) {
      talker.warning('Watch channel unavailable; skipping sendObsScenesToNative');
      return;
    }
    try {
      await _channel.invokeMethod("flutterToWatch", {
  ...
  Future<void> sendObsSourcesToNative(String data) async {
    if (!await _ensureChannelAvailable()) {
      talker.warning('Watch channel unavailable; skipping sendObsSourcesToNative');
      return;
    }
    try {
      await _channel.invokeMethod("flutterToWatch", {
  ...
  Future<void> sendSeConnectedToNative({required bool isConnected}) async {
    if (!await _ensureChannelAvailable()) {
      talker.warning('Watch channel unavailable; skipping sendSeConnectedToNative');
      return;
    }
    try {
      await _channel.invokeMethod("flutterToWatch", {
  ...
  Future<void> sendSeActivityToNative(SeActivity activity) async {
    if (!await _ensureChannelAvailable()) {
      talker.warning('Watch channel unavailable; skipping sendSeActivityToNative');
      return;
    }
    try {
      await _channel.invokeMethod("flutterToWatch", {
  ...
  Future<void> sendLiveStatusToNative({required bool isLive}) async {
    if (!await _ensureChannelAvailable()) {
      talker.warning('Watch channel unavailable; skipping sendLiveStatusToNative');
      return;
    }
    try {
      await _channel.invokeMethod("flutterToWatch", {
